...
<HEAD>
	...
	function writeReturnhref()
	{
		...
		document.write('<A class="aR" style="color:#2CBCD4;" href="' + httpip + '" onclick="resetCookie();">返回登录页面</A>');
	}
...
<BODY style="TEXT-ALIGN: center; valign: middle" vLink="#000000" aLink="#000000" link="#000000" 
leftMargin="0" topMargin="0" onload="LoadFrame()" marginheight="0" marginwidth="0">
<SCRIPT language=JavaScript type=text/javascript>
function LoadFrame()
{
	...
	with ( getElById('ConfigForm') )
	{
		...
		{
		<% end if %>		
		if ( (parseInt(lefttime) > 0) || ((isPageUpdate == 1) && (parseInt(lefttime) == 0)) )
		{
...
function pageHeight()
{
	...
	<input TYPE="hidden" NAME="result_startvalue" value="99">
	...
	document.write('<table class="tblR" width="808" height="540" align="center" background="/img/register_bg.png" style="margin-top:' + winHeight + 'px">');
	...
	<TD colspan="3">
	<% end if %>
		<TABLE align="center" class="regTbl">
		<TBODY>
		<TR>
		<TD width="8%">&nbsp;</TD>
		...
		<TD style="FONT-SIZE: 12px" colSpan=2>GPON上行智能家庭网关业务注册提示：</TD>
		</TR>
		<% else %>
		<TD style="FONT-SIZE: 12px" colSpan=2>EPON上行智能家庭网关业务注册提示：</TD>
		</TR>
		<% end if %>
		<% end if %>
		<TR>
		<TD width="8%">&nbsp;</TD>
		<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes" then %>
		<TD style="FONT-SIZE: 12px" colSpan=2>1.按要求插紧光纤后再确认终端光信号灯熄灭。</TD>
		</TR>
		<% elseif tcWebApi_get("WebCustom_Entry", "isCMCCSupported", "h") = "Yes" then %>
		<TD style="FONT-SIZE: 12px" colSpan=2>1.请插紧光纤并确认光信号灯已处于熄灭状态</TD>
		</TR>
		<% elseif tcWebApi_get("WebCustom_Entry", "isCTPONCYSupported", "h") = "Yes" then %>
		<TD style="FONT-SIZE: 12px" colSpan=2>1.请插紧“光纤E/G”接口的光纤，检查并确认光信号灯已处于熄灭状态</TD>
		</TR>
		<% else %>
		<TD style="FONT-SIZE: 12px" colSpan=2>1.请插紧“网络E/G”接口的光纤，检查并确认光信号灯已处于熄灭状态</TD>
		</TR>
		<% end if %>
		<% if tcWebApi_get("WebCustom_Entry", "isCTSIMCARDSEPARATIONSupported", "h") = "Yes" then %>
		<% if tcWebApi_get("SimCard_Entry", "cardExist", "h") = "1" then %>
		...
		<TD width="8%">&nbsp;</TD>
		<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes" then %>
		<TD style="FONT-SIZE: 12px" colSpan=2>2.光信号常亮后输入LOID和PASSWORD（密码）进行注册。密码可以为宽带帐号（不带@后缀）、电话号码（不加区号）或LOID（纯ITV接入）。</TD>
		</TR>
		<% else %>
		<TD style="FONT-SIZE: 12px" colSpan=2>2.准确输入<a id="hint2Text">“逻辑ID”和“密码”</a>，点击“确定”进行注册或插入家庭<br/>&nbsp; 网关卡后重启设备</TD>
		</TR>
		<% end if %>
		<% end if %>
		<% else %>
		...
		<% elseif tcWebApi_get("WebCustom_Entry", "isCMCCSupported", "h") = "Yes" then %>
		<TD style="FONT-SIZE: 12px" colSpan=2>2.准确输入“Password”，点击“确定”进行注册</TD>
		</TR>
		<% else %>
		<TD style="FONT-SIZE: 12px" colSpan=2>2.准确输入<a id="hint2Text">“逻辑ID”和“密码”</a>，点击“确定”进行注册</TD>
		</TR>
		<% end if %>
		<% end if %>
		<% if tcWebApi_get("WebCustom_Entry", "isCTSIMCARDSEPARATIONSupported", "h") = "Yes" then
		...
		<TD width="8%">&nbsp;</TD>
		<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes" then %>
		<TD style="FONT-SIZE: 12px" colSpan=2>2.在注册过程中不要随意拨插光纤（10分钟内）。</TD>
		</TR>
		<% else %>
		<TD style="FONT-SIZE: 12px" colSpan=2>2.在注册及业务下发过程中（10 分钟内）不要断电、不要拔光纤</TD>
		</TR>
		<% end if %>
		<TR>
		<TD width="8%">&nbsp;</TD>
		<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes" then %>
		<TD style="FONT-SIZE: 12px" colSpan=2>3.本注册功能仅用于新设备的认证及业务下发，已正常在用设备请勿重新注册。</TD>
		</TR>
		<% else %>
		<TD style="FONT-SIZE: 12px" colSpan=2>3.本注册功能仅用于新设备的认证及业务下发，已正常在用设备请勿重新注册</TD>
		</TR>
		...
		<TD width="8%">&nbsp;</TD>
		<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes" then %>
		<TD style="FONT-SIZE: 12px" colSpan=2>3.在注册过程中不要随意拨插光纤（10分钟内）。</TD>
		</TR>
		<% else %>
		<TD style="FONT-SIZE: 12px" colSpan=2>3.在注册及业务下发过程中（10 分钟内）不要断电、不要拔光纤</TD>
		</TR>
		<% end if %>
		<TR>
		<TD width="8%">&nbsp;</TD>
		<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes" then %>
		<TD style="FONT-SIZE: 12px" colSpan=2>4.本注册功能仅用于新设备的认证及业务下发，已正常在用设备请勿重新注册。</TD>
		</TR>
		<% else %>
		<TD style="FONT-SIZE: 12px" colSpan=2>4.本注册功能仅用于新设备的认证及业务下发，已正常在用设备请勿重新注册</TD>
		</TR>
		...
		<TD width="8%">&nbsp;
			<input type="hidden" name="romfileselect" value="">
			<input type="hidden" name="ori_romfileselect" value="<%TCWebApi_get("System_Entry", "romfileSelect", "s")%>">
		</TD>
		...
		<% end if %>
		<TD width="60%">
		<INPUT id="broadbandusername" style="WIDTH: 120px; FONT-FAMILY: '宋体'" name="broadbandusername" height="20" 
		value="<% If TCWebApi_get("deviceAccount_Entry", "userName", "h") <> "N/A" then TCWebApi_get("deviceAccount_Entry", "userName", "s") end if %>">
		&nbsp;<FONT color=red>*</FONT>
		</TD>
		</TR>
		<% end if %>
		...
		<% end if %>
		<TD>
			<INPUT id="customer_ID" style="WIDTH: 120px; FONT-FAMILY: '宋体'"
			<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") <> "Yes" then %>
			<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") <> "Yes" then %>
			type="password"
			<% end if %>
			...
			TCWebApi_get("deviceAccount_Entry", "userPasswordDEV", "s") end if %>"> 
		</TD>
		</TR>
		<TR id="msgwait">
		<TD width="8%">&nbsp;</TD>
		...
		<TD colSpan=2>
			...
			<TR>
			<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes" then %>   
			<TD width="20%" align=right ><INPUT id="btnOKR" onclick="onOK()" type=button value="确定"></TD>
			<% else %>
			<TD width="45%" align=right ><INPUT id="btnOKR" onclick="onOK()" type=button value="确定"></TD>
			<% end if %>         
			<% if tcWebApi_get("WebCustom_Entry", "isCTPONC9Supported", "h") = "Yes" then %> 
			<TD width="10%"><INPUT id="btnSetting" onclick="onSetting()" type=button value="高级设置"></TD>
			<% else %>
			<TD width="10%">&nbsp;</TD>
			<% end if %>
<% if tcWebApi_get("WebCustom_Entry", "isCTSIMCARDSEPARATIONSupported", "h") = "Yes" then 
if tcWebApi_get("SimCard_Entry", "cardExist", "h") = "1" then %>
			<TD width="45%" align=left ><INPUT id="btnCancelR" onclick="onReset()" type="button" value="重置"></TD>
			</TR>
			</TBODY>
			</TABLE>
		</TD>
		...
		<TR align=center>
		<TD width="8%">&nbsp;</TD>
		<TD colSpan=2><INPUT id=SerConfig onclick="onOK()" type=button value=业务配置 name=SerConfig></TD>
		</TR>
		</DIV>
		</TBODY>
		</TABLE>
...
<% end if
			...
			<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes" then %>  
			<TD width="20%" align=left ><INPUT id="btnCancelR" onclick="onReset()" type="button" value="重置">
			<% else %>
			<TD width="45%" align=left ><INPUT id="btnCancelR" onclick="onReset()" type="button" value="重置">
			<% end if %>
			<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes" then %>    
			<TD width="20%" align=left ><INPUT id="btnCancelR" onclick="btnselfCheck()" type="button" value="终端自检">       	
			<% end if %>
			</TD>
			<% if tcWebApi_get("WebCustom_Entry", "isCTSCSupported", "h") = "Yes" then %>
			<TD width="20%" align=left ><INPUT id="btnCancelR" onclick="restore()" type="button" value="恢复出厂"></TD>
			<% end if %> 
			</TR>
			</TBODY>
			</TABLE>
...
<% end if %>
	...
	document.write('<table width="808" height="540" align="center" background="/img/register_bg.png" style="margin-top:' + winHeight + 'px">');
	...
	<TD>
		...
		<TR>
		<TD>&nbsp;&nbsp;&nbsp;&nbsp;</TD>
		<TD><DIV id="loading"><strong id="loadcss"></strong></DIV>&nbsp;</TD>
		</TR>
		<TR>
		<TD></TD>
		<TD><DIV id=tdResult class="text-center" style="FONT-SIZE: 13px"></DIV></TD>
		</TR>
		<TR id=trReturn style="DISPLAY: none"><TD align=middle colSpan=2><INPUT id=btnReturn onclick=onReturn(0); type=button value=返回></TD></TR>
		...
		<TD colSpan=2>
			...
			<TBODY>
			<TR>
			<TD width="45%" align=right><INPUT id=btnReturn1 onclick=onReturn(1); type=button value=返回></TD>
			<TD width="10%">&nbsp;</TD>
		...
		<TD colSpan=2>
			...
			<TD width="62%" align=left>
				<INPUT id="TestCode" style="WIDTH: 120px; FONT-FAMILY: '宋体'" type="password" name="TestCode" height="20" value="">&nbsp;<FONT color=red>*</FONT>
			</TD>
			</TR>
			</TBODY>
			</TABLE>
		...
		<TD colSpan=2>
			...
			<TBODY>
			<TR>
			<TD width="45%" align=right><INPUT id="btnSubmit" onclick="onSubmit()" type=button value="提交"></TD>
			<TD width="10%">&nbsp;</TD>
			<TD width="45%" align=left><INPUT id="btnCancelR" style="WIDTH: 60px;HEIGHT: 20px" onclick="onCancel()" type="button" value="取消" class="BtnCnl"></TD>
			</TR>
			</TBODY>
			</TABLE>
		</TD>
		...
		<% else %>
		<TD align=middle colSpan=2><INPUT id=btnReturn1 onclick=onReturn(1); type=button value=返回></TD>
		</TR>
		<% end if %>
		<TR id=trTime style="DISPLAY: none">
		<TD>&nbsp;</TD>
		<TD id=tdTime style="FONT-SIZE: 13px">
			<DIV id="regStatus" style="DISPLAY: none">
			<iframe src="/cgi-bin/regstatus.asp" onload='ifame_load_finish()' id="statusfrm" name="statusfrm" frameborder="0" width="0" height="0"></iframe>
			</DIV>
		</TD>
		</TR>
		</TBODY>
	...
	function showMsg()
	{
		...
		{			
			...
			if ( 1 == gConnectITMS )
			{
				...
				if ( (gITMSStep < gITMSMsgSize) || (1 == msg_show) )
				{
					statusFlag = 0;
					return msg_tmp;
				}
			...
			{
				if ( (percent < 30) || (pppup == 'N/A') || ((30 == percent) && ('1' == rs) && (pppup != 'N/A')) )
				{
					statusFlag = 0;
					percent = 30;	//change to 30%
					<% if tcWebApi_get("WebCustom_Entry", "isCTFJSupported", "h") = "Yes" then %>
				...
				if ( (acsresult != "1") && (acsresult != "2") || ((isOLTReg = 1) && (++cwmpRetry <= 72)) )
				{
					statusFlag = 0;
			...
			else if ( rstatus == "0" )
			{
				if ( rresult == "1" )
				{
					...
					else
					{
						infoStr = "ITMS平台业务数据下发成功。";
						<% if tcWebApi_get("WebCustom_Entry", "isCMCCSupported", "h") = "Yes" then %>
						infoStr = "RMS平台数据下发成功，共下发了" + serviceList.toString() + " " + serviceNum.toString() + "个业务。";
						<% end if %>
					}
					return infoStr;
				}
					...
					else
					{
						...
						switch ( serviceNameId )
						{
							...
							case 4:
								...
								document.cookie = "RegServiceINTERNET=1";
								<% if tcWebApi_get("WebCustom_Entry", "isCMCCSupported", "h") = "Yes" then %>
								serviceNameStr = "组播类视频、上网";
								<% else %>
								serviceNameStr = "iTV、上网";
								<% end if %>
								break;
							case 5:
								document.cookie = "RegServiceIPTV=1";
								document.cookie = "RegServiceVOIP=1";
								<% if tcWebApi_get("WebCustom_Entry", "isCMCCSupported", "h") = "Yes" then %>
								serviceNameStr = "组播类视频、语音";
								<% else %>
								serviceNameStr = "iTV、语音";
								<% end if %>
								break;
							case 6:
								document.cookie = "RegServiceINTERNET=1";
								document.cookie = "RegServiceVOIP=1";
								serviceNameStr = "上网、语音";
								break;
							case 7:
								document.cookie = "RegServiceIPTV=1";
								document.cookie = "RegServiceINTERNET=1";
								document.cookie = "RegServiceVOIP=1";
								<% if tcWebApi_get("WebCustom_Entry", "isCMCCSupported", "h") = "Yes" then %>
								serviceNameStr = "组播类视频、上网、语音";
								<% else %>
								serviceNameStr = "iTV、上网、语音";
								<% end if %>
								break;								
							default:
