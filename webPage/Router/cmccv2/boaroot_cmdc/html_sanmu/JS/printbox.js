...
//style=\"word-wrap:break-word\"
	PrintBuf+=('<TABLE border=\'1\' name="tabStaticRouteV4" class="tblList" width=\"' + TabWidth + '\"><thead>');
	
	var tempArray = arrMsgInfo[0];	

	//输出提示行
	PrintBuf+=("<TR class=\"trStyle2\">");
	for(j=3;j<tempArray.length-2;j++)
    {    
	    width = ' width=\"' + arrMsgInfo[0][j] + '\" ';
		strTDStart = '<TD align=\"center\"'  + width + backColor + '>';
	...
	{	
		...
		for(j=3;j<tempArray.length-2;j++)
		{
			...
			if (arrMsgInfo[2][j] != "")
    	    {
			    ...
			    strTemp = strTDStart + getDisplayText(strVal, 15);
    	    }
    		else
    		{
				if (arrMsgInfo[i][j] != '')
    		    {
					strTemp = strTDStart + getDisplayText(arrMsgInfo[i][j], 15);
				}
			    else
			    {
			        strTemp = strTDStart +'&nbsp;';
	...
	PrintBuf += ('</TBODY></TABLE>');
  PrintBuf +=  '<TR><TD style=\"padding: 32px 0 60px;text-align: center;\"><INPUT type=\"button\" name="BtnAddv4" class="BtnAdd"  value = \"添加\" onclick = \"webClickAdd('+'\''+strDomainName+'\''+','+'\''+strUrl+'\''+');\"> \n';
	PrintBuf +=  '<INPUT type=\"button\" name="BtnDelv4" class="BtnAdd" style=\"margin-left: 24px;\" value = \"删除\" onclick = \"webClickRemove();\"></TD></TR>';
