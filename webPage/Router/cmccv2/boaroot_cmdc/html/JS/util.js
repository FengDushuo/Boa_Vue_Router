var countTime = 0; //计时器，单位：秒
var outTime = 15;  //超时时间：15分钟
var event_interval = window.setInterval(go, 1000); //超时15分钟自动退出登录
function go() {
    countTime++;
    if (countTime == outTime * 60) {
...
// 页面帮助显示/隐藏
function showHelp() {
    var show = $(".helpDesc").css("display");
    if (show == "none") {
        $(".helpDesc").css("display", "")
    }
    else {
        $(".helpDesc").css("display", "none")
    }
}
...
function isTimeOut() {
    ...
    if (currentTime - lastTime >= timeOut) { // 判断时间间隔是否大于15分钟
        logout();
    }
}
...
function checkSSID(s) {
    ...
    for (var i = 0; i < s.length; i++) {
        if (s.charCodeAt(i) >= 10000) { //1个中文字符等于2个英文字符
            len += 2; // 限制20位的时候用这个
        }
        else if (reg.test(s)) {
            len++;
...
function isValidNamePPPoE(e) {
    ...
    if (t == null) {
        ...
        if (e.indexOf('&lt,') > -1 || e.indexOf('&gt,') > -1 || e.indexOf('&#') > -1) {
            return false
        }
        return true
    }
...
function isValidIPandURLAddr(address) {
    var isip = isValidIpAddress(address);
    if (isip) {
        return true;
    }
...
function checkGW(gw) {
    var exp = /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-4])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-4])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-4])\.([1-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-4])$/;
    var reg = gw.match(exp);
    if (reg == null) {
        return false;
    } else {
        return true;
...
function setDisplay(sId, sh) {
    ...
    if (getElement(sId)) { getElement(sId).style.display = status; }
}
/*end of HGW 2008.1.10 For A36D04370 FireFox 显示 V100R001C02B013 by w00104696*/
...
function FinishLoad() {
    ...
    {
        $("[type='button']").css({"background" : "#2CBCD4", "border" : "none", "color" : "#FFFFFF"});
        $(".BtnAdd").css({"background" : "#A5CF44", "border" : "none", "color" : "#FFFFFF"});
        $(".BtnDel").css({"background" : "#E96979", "border" : "none", "color" : "#FFFFFF"});
        $(".BtnBCnl").css({"background" : "#FFFFFF", "color" : "#2CBCD4", "border" : "1px solid #2CBCD4"});
    	
        $("[type='button'], .BtnAdd, .BtnDel, .BtnBCnl").css({
            "width" : "90px",
            "height" : "30px", 
            "font-size" : "11px",
            "line-height" : "11px",
...
function getRadioVal(sId) {
    ...
    for (i = 0; i < item.length; i++) {//alert(item[i].checked)
...
function setheight(setelm, elm1, elm2) {
    var h = document.documentElement.clientHeight;

    if (elm1 != "") {
        h -= getElById(elm1).clientHeight;
    }

    if (elm2 != "") {
        h -= getElById(elm2).clientHeight;
    }

    setElementStyle(setelm, "height", h);
}
...
function set_action(input)
{
	...
	{
		ac=document.forms[i].action;
		var raw_url=ac.substring(0,ac.indexOf('?post_par='));
		if(raw_url.length)
		{
			console.log("already set");
			url=raw_url+input;
			$(document.forms[i]).attr("action",url);	
		}
		else
		{
			console.log("not set");
			url=document.forms[i].action+input;
			$(document.forms[i]).attr("action",url);			
		}
